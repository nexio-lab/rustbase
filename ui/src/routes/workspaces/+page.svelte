<script lang="ts">
	...
	// a separate /workspaces/new route) means the user never loses the
	// list context.
	...
	async function submitCreate(e: SubmitEvent) {
		...
		createError = null;
		try {
			const created = await api.post<Workspace>('/api/workspaces', { id: newId, name: newName });
			workspaces = [...workspaces, created].sort((a, b) => a.id.localeCompare(b.id));
			creating = false;
		} catch (e) {
	...
	function openRealm(workspace: Workspace) {
		// Phase 2 wires per-workspace pages. For now, just stash the chosen
		// workspace in the URL — a follow-up branch picks it up.
...
<div class="mb-6 flex items-end justify-between">
	<div>
		<h1 class="text-2xl font-semibold tracking-tight text-slate-900">Workspaces</h1>
		<p class="mt-1 text-sm text-slate-500">
			Identity boundaries. Each workspace owns its own user pool, OAuth providers, and apps.
		</p>
	</div>
	{#if !creating}
		<button class="btn-primary" onclick={openCreate}>+ New workspace</button>
	{/if}
</div>

{#if creating}
	<form onsubmit={submitCreate} class="card mb-6 max-w-lg space-y-4">
		<h2 class="text-lg font-semibold text-slate-900">Create workspace</h2>
		{#if createError}
		...
		<div>
			...
			<p class="mt-1 text-xs text-slate-500">
				lowercase letters, digits, hyphens. The master workspace <code>master</code> is reserved.
			</p>
		</div>
		<div>
...
{:else if workspaces.length === 0}
	<div class="card text-center text-slate-500">
		...
		<p class="mt-1 text-xs">Click <strong>New workspace</strong> to create one.</p>
	</div>
{:else}
