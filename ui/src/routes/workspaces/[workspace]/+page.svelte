<script lang="ts">
	...
	async function load() {
		...
		listError = null;
		try {
			apps = await api.get<App[]>(`/api/workspaces/${workspace}/apps`);
		} catch (e) {
	...
	// Reruns whenever the URL workspace slug changes (so /workspaces/acme → /workspaces/widgets
	...
	async function submitCreate(e: SubmitEvent) {
		...
		createError = null;
		try {
			const created = await api.post<App>(`/api/workspaces/${workspace}/apps`, {
				id: newId,
				name: newName
			});
	...
	function openApp(app: App) {
		goto(`/workspaces/${workspace}/apps/${app.id}`);
	}
</script>
