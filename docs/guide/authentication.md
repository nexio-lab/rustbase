...
Send `Authorization: Bearer <access_token>` on every authenticated call. Refresh tokens are exchanged at `/auth/refresh`:
...
The response is a new access + refresh pair. The old refresh token is **invalidated immediately**.
...
```http
POST /api/realms/:realm/auth/otp/request    { "email": "u@acme.com" }
POST /api/realms/:realm/auth/otp/login      { "email": "u@acme.com", "code": "123456" }
```
...
```http
POST /api/realms/:realm/auth/totp/enroll       [user token]
  → returns { "secret": "...", "qr_url": "otpauth://..." }

POST /api/realms/:realm/auth/totp/confirm      [user token]
  { "code": "123456" }
  → 200, TOTP is now active for this user

POST /api/realms/:realm/auth/totp/disable      [user token]
  { "code": "123456" }
```
...
Realm admins can force-unenroll a user with `DELETE /api/realms/:realm/users/:id/totp` (recovery flow).

## OAuth2 / OIDC

Built on the [`oauth2`](https://docs.rs/oauth2) crate. Providers are configured **per realm** by an admin:

```http
PUT /api/realms/:realm/auth/oauth/providers/google
{
  "client_id":     "...",
...
```http
GET /api/realms/:realm/auth/oauth/google/authorize?redirect_uri=https://app/cb
  → 302 to Google with a state cookie set

POST /api/realms/:realm/auth/oauth/google/callback
  { "code": "...", "state": "...", "redirect_uri": "https://app/cb" }
  → 200  { access_token, refresh_token, user }
```

State is single-use and bound to the realm. On first sign-in via OAuth, a user row is created with `has_password = false`. Subsequent logins match by `email`.

## Admin tokens

Master admins call `/_/auth/admin/login`; realm admins call `/api/realms/:realm/auth/admin/login`. Tokens carry a `role` claim (`master_admin`, `realm_admin`, `app_admin`, `user`). Every protected handler enforces:
